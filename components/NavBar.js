  ...
  return (
    <div className="flex flex-col max-w-8xl m-auto z-50">
      <div className="fixed top-0 left-0 bg-white h-24 w-full flex flex-row justify-between items-center p-8">
        ...
        <Link href="/" className="text-2xl md:w-1/3">Chotto</Link>
        {/* RIGHT - BARS */}
        ...
        <div className="hidden md:block">
            <Image src="/icons/instagram-icon.svg" width={25} height={25} alt=""></Image>
        </div>
      </div>
      {/* TOGGLED MENU */}
      <div
      ...
      {/* BOTTOM SOCIALS */}
      <div className={isToggleMenuOpen ? "fixed z-50 overflow-hidden bg-white bottom-0 left-0 h-24 w-full flex justify-center items-center md:hidden" : "hidden"}>
        <Image src="/icons/instagram-icon.svg" width={25} height={25} alt=""></Image>
      </div>
    </div>
  );
